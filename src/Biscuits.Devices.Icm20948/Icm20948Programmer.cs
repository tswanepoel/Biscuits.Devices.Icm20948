namespace Biscuits.Devices.Icm20948
{
    ...
    public class Icm20948Programmer
    {
        // eMD-SmartMotion-ICM20948-1.1.1/EMD-App/src/ICM20948/icm20948_img.dmp3a.h
        // https://invensense.tdk.com/developers/software-downloads/#motion-libraries
        private static readonly byte[] _dmp3aImage = [
            /* bank # 0 */
        ...
        private readonly ILogger<Icm20948Programmer> _logger;

        public Icm20948Programmer(ILogger<Icm20948Programmer>? logger = null)
        {
            _logger = logger ?? NullLogger<Icm20948Programmer>.Instance;
        }

        public void LoadFirmware(Icm20948Spi icm20948)
        {
            _logger.LogDebug("Load firmware is starting.");
            ...
            for (int position = 0; position < _dmp3aImage.Length / chunkSize; position += chunkSize)
            {
                ...
                var addr = (ushort)(0x0090/*DMP_LOAD_START*/ + position);
                var memBank = (byte)(addr >> 8);
                var memStartAddr = (byte)(addr & 0xFF);

                icm20948.SetMems(memBank, memStartAddr, buffer);
            }

            _logger.LogDebug("Load firmware is verifying.");
            ...
            for (int position = 0; position < _dmp3aImage.Length / chunkSize; position += chunkSize)
            {
                ...
                icm20948.GetMems(memBank, memStartAddr, buffer2);

                Span<byte> s1 = buffer2.Slice(0, length);
                Span<byte> s2 = _dmp3aImage.AsSpan(position, length);
            ...
            _logger.LogDebug("Load firmware is completed.");
        }
    }
}
